...
This was measured rather than assumed, and the numbers are larger than they
look. Counting how much lane a cell can actually see, the best cells at
Knitting Norah's reach see 288 pixels of road against a median cell's 120, and
at Binocular Bill's reach 944 against 448. Where a defender stands is worth
more than a tier of upgrades.

It went unnoticed for a long time because every board the harness measured was
generated by spacing defenders evenly along the lane, which is precisely the
placement rule that cannot discover a hairpin: it gives each defender one
stretch of road and no two defenders the same crowd. Those boards all lost
while the game was being finished comfortably by hand. The board that won
packed thirteen defenders into a 240-pixel box beside the first hairpin, which
covers lane distances 160-400 *and* 840-960 because the street doubles back
through it. It is kept in `src/sim/builds.ts` as `corner`, exactly as played.

## A board of one kind of defender should lose

Mixing is rewarded, and deliberately so. A knot of nothing but knitters loses
late even when it is placed on the best cells in the board and upgraded well --
that was measured at several anchors, and it is the intended shape of the game
rather than a balance fault to be corrected.

The reason is that most of what the defenders offer only exists between them.
Coffee Clara multiplies her neighbours and does nothing alone. Baking Barbara
is paid for density, which needs something else holding the crowd together.
Binocular Bill wants range that the short defenders cannot use. A board of one
kind collects none of that, and late rounds are where the difference tells.

This is the same idea as the rest of this document approached from the other
side: there is no counter table, so no defender is the answer to a
troublemaker -- but defenders are very much the answer to each other.
