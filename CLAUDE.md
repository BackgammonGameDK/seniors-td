...
Three limits worth knowing. A loadout can only buy, so a run where anything was
sold records as though it never was -- the box says so when that happens. A
tower built again on a cell where one had stood, such as a garden wall knocked
down and put back, records as that one tower, so a harness may never pay for
the second -- the box says that too. And **Restart clears the recording**, so
press `L` before starting again.
